...
<head>
    ...
    <style>
        ...
        @media (min-width: 1024px) {
            /* Maximize the height of the scrollable chemical list */
...
<body class="text-slate-800 flex flex-col min-h-screen">
    ...
    <!-- Header: Royal Blue Background - Using max-w-7xl for a wide, defined header -->
        ...
        <div class="max-w-7xl mx-auto w-full"> 
            <div class="flex flex-col items-center justify-center mb-6 border-b border-blue-700/50 pb-4">
                ...
                <img 
                    ...
                    class="w-full max-w-72 h-auto rounded-lg object-contain mb-4 border-2 border-white shadow-xl bg-white p-2" 
                    onerror="this.onerror=null; this.src='https://placehold.co/300x75/FFFFFF/000000?text=SMC+TAGUM+LAB';"
                />
    ...
    <!-- Main Content - Set to w-full to stretch the content, using wide horizontal padding -->
    <main class="flex-1 w-full px-4 md:px-8 py-6 grid grid-cols-1 lg:grid-cols-4 gap-6">
        
        <!-- Left Column: Search Results (Takes 1/4 width on desktop) -->
        <div id="chemical-list-container" class="bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden flex flex-col h-[500px] lg:h-[calc(100vh-140px)]">
            ...
            <div id="chemical-list" class="overflow-y-auto flex-1 p-3 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-2 gap-2"> 
                <!-- Chemical buttons go here -->
            </div>
        </div>

        <!-- Right Column: Locator Map & Info (Takes 3/4 width on desktop) -->
...
208. MALTOSE MONOHYDRATE 92%,,,,,,LEFT,,24
        ...
        function getStaticSafetyData(chemicalName) {
            ...
            if ((chemicalName.includes('acid') && !chemicalName.includes('amino')) || chemicalName.includes('bisulfate')) {
                type = "Strong Acid/Corrosive"; // ACID Classification
                ...
                type = "Strong Base/Corrosive"; // BASE Classification (Updated for clarity)
            ...
            } else if (chemicalName.includes('alcohol') || chemicalName.includes('acetone') || chemicalName.includes('hexane') || chemicalName.includes('chloroform') || chemicalName.includes('methanol') || chemicalName.includes('benzene')) {
            ...
            } else if (chemicalName.includes('lead') || chemicalName.includes('mercury') || chemicalName.includes('cadmium') || chemicalName.includes('ferricyanide')) {
                type = "Toxic Heavy Metal Salt";
                data.safety = "<h4>**Hazards**</h4><p class='mt-2'>Highly Toxic. Cumulative poison that can harm the nervous system and kidneys. Avoid all contact and inhalation.</p><h4>**PPE**</h4><p class='mt-2'>Double-gloving (Nitrile), chemical apron, and dedicated respirator if handling powders. Use only in a designated area.</p><h4>**Handling**</h4><p class='mt-2'>Minimize disturbance of powder to prevent dust. Use a wet wipe-down procedure for spills. Never heat without proper ventilation.</p>";
                data.disposal = "<h4>**Pre-Disposal Check**</h4><p class='mt-2'>These are regulated hazardous wastes. DO NOT dispose of via the sink or regular trash.</p><h4>**Procedure**</h4><p class='mt-2'>Collect all solid and liquid waste (including contaminated rinse water) in a clearly labeled 'Heavy Metal Waste' container. Follow specific hazardous waste collection procedures for your institution.</p>";
                data.uses = "Complex ion formation, qualitative analysis tests, or specific catalyst preparation.";
            } else if (chemicalName.includes('nitrate') || chemicalName.includes('sulfate') || chemicalName.includes('chloride') || chemicalName.includes('carbonate')) {
                type = "Inorganic Salt/Oxidizer";
                if (chemicalName.includes('permanganate') || chemicalName.includes('peroxide') || chemicalName.includes('nitrite')) {
                    type = "Oxidizer/Reactive";
                }
                data.safety = "<h4>**Hazards**</h4><p class='mt-2'>Generally low immediate hazard, but toxic if large amounts are ingested. Oxidizers (like Permanganate) must be kept away from organic materials (wood, paper, solvents).</p><h4>**PPE**</h4><p class='mt-2'>Standard lab coat and eye protection. Use gloves when handling powders to avoid skin exposure.</p><h4>**Handling**</h4><p class='mt-2'>Avoid generating dust. Store oxidizers separately from flammables and reducing agents.</p>";
                data.disposal = "<h4>**Pre-Disposal Check**</h4><p class='mt-2'>Most non-heavy metal salts are safe for sink disposal. Oxidizers must be deactivated first.</p><h4>**Procedure**</h4><p class='mt-2'>If non-regulated, dissolve in excess water and flush down the drain with running water. For oxidizers, consult SOP to reduce them to a safer compound before disposal.</p>";
        ...
        // Helper function to get color class based on chemical type
        function getTypeBadgeClass(type) {
            switch (type) {
                case 'Strong Acid/Corrosive': return 'bg-red-600 text-white';
                case 'Strong Base/Corrosive': return 'bg-blue-600 text-white'; 
                case 'Flammable/Volatile Solvent': return 'bg-orange-500 text-white';
                case 'Toxic Heavy Metal Salt': return 'bg-purple-600 text-white';
                case 'Oxidizer/Reactive': return 'bg-pink-600 text-white';
                case 'Inorganic Salt/Oxidizer': return 'bg-green-500 text-white';
        ...
        function renderChemicalList(chemicals) {
            ...
            chemicals.forEach(chem => {
                ...
                // Adjusted grid to handle wide screens: lg:grid-cols-2 is inside a 1/4 column
        ...
        function renderSelectedChemicalPanel() {
            ...
            const chemicalData = getStaticSafetyData(selectedChemical.name);
            const badgeClass = getTypeBadgeClass(chemicalData.type);
                    ...
                    <div class="bg-gradient-to-r from-blue-50 to-white border-l-4 border-yellow-500 text-blue-900 p-4 rounded-r-lg shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div class="flex flex-col gap-2">
                            <div class="flex flex-wrap items-center gap-3">
                                ...
                                <span class="block font-bold text-2xl leading-tight text-blue-900">${selectedChemical.name}</span>
                                
                                <!-- Classification Badge Added Here -->
                                <span class="text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider shadow-md ${badgeClass}">
                                    ${chemicalData.type}
                                </span>
                            </div>
                            <span class="text-sm text-blue-700 mt-1 sm:ml-16">
